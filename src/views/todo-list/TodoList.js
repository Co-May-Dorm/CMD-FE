...
const TodoList = () => {
    ...
    const statusFormTask = useSelector(state => state.TodoListReducer.showFormTask)
    ...
    // console.log(tasks)
    //render task item
    const renderTaskItem = () => tasks.map((item, key) => <TaskItem key={key} task={item} />)
    ...
    return (
        <>
                    ...
                    <div className="col-sm">
                        {sizeWindown[0] < 1060 ? <div className="d-flex justify-content-evenly"> <button className=" btn btn-outline-primary fw-bold shadow-button">Tùy chọn <IoIosArrowDropdown size={25} /></button></div> :
                            <div className="d-flex justify-content-evenly">
                                <button type="button" className="btn btn-outline-darkBlue btn-sm fw-bigBold fs-8" onClick={() => showFromTask("NewTask")} data-mdb-ripple-color="dark">Tạo việc</button>
            ...
            <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                ...
                <FormTask show={statusFormTask} name_form={nameForm} onHide={() => showFromTask("")} />
