...
const NewTask = React.memo((props) => {
  ...
  const [showListEmployeesInvolve, setShowListEmployeesInvolve] =
    useState(false);
  ...
  //state store
  const listEmployeeSearch = useSelector(
    (state) => state.TodoListReducer.listEmployeeSearch
  );
  const startNewDateTask = useSelector(
    (state) => state.TodoListReducer.startDateNewTask
  );
  const endNewDateTask = useSelector(
    (state) => state.TodoListReducer.endDateNewTask
  );
    ...
    if (props.name_form === constant.CREATE_TASK) {
      return (
        <Modal.Footer>
          ...
          <Button onClick={addNewTask}>Lưu</Button>
        </Modal.Footer>
      );
    } else {
      return (
    ...
    for (let i = 0; i <= index; i++) {
      els.push(
        <div key={i} className="col-sm-3 d-flex flex-column w-25">
          <button
            style={{
              backgroundColor: i <= levelPrioritize ? arrayColor[i] : null,
              borderColor: i <= levelPrioritize ? arrayColor[i] : null,
            }}
            onClick={() => setLevelPrioritize(i)}
  ...
  const searchEmployee = (event, type) => {
    ...
    const delaySearch = setTimeout(() => {
      dispacth(
        todoListAction.searchEmployee({ typeSearch: type, request: req })
      );
    }, 1000);
  ...
  const addNewTask = () => {
    ...
    dispacth(todoListAction.createNewTask(data));
    dispacth(todoListAction.dispatchTaskRequest({ page, search }));
    props.onHide();
  };
  ...
  return (
    <>
          ...
          <div>
            <div className="row ms-5 me-5 mb-2">
              ...
              <div className="d-flex form-control">
                <input
                  ...
                  className={"ps-2 w-100 text-blue"}
                ></input>
              </div>
            </div>
            <div
              className="row ms-5 me-5 mb-2 position-relative"
              ref={wrapperRef}
            >
              <span
                className="fw-bold pe-0 ps-0 fs-4"
                onClick={() => setShowListEmployees(false)}
              >
                Người được giao
              </span>
              <div
                className="d-flex form-control rounded-top"
                onClick={() => setShowListEmployees(!showListEmployees)}
              >
                <input
                  value={employee === null ? "" : employee.name}
                  type={"search"}
                  onChange={(event) =>
                    searchEmployee(event, constant.DISPATCH_EMPLOYEE_SEARCH)
                  }
                  style={{ border: "none", outline: "none" }}
                  className={"ps-2 w-100 text-blue"}
                />
              ...
              {showListEmployees ? (
                <ul
                  className="list-group pe-0 position-absolute position-absolute top-100 start-0 bg-white"
                  role={"listbox"}
                  style={{
                    overflowY: "auto",
                    maxHeight:
                      listEmployeeSearch.length === 0 ? "40px" : "200px",
                    cursor: "pointer",
                    zIndex: "2",
                  }}
                >
                  {listEmployeeSearch.length === 0 ? (
                    ...
                    listEmployeeSearch.map((item, id) => (
                      <li
                        onClick={() => choseEmployee(item)}
                        key={id}
                        role={"option"}
                        className="list-group-item list-group-item-action"
                      >
                        <span className="mx-auto text-blue">
                          {" "}
                          <FaUserAlt size={15} className="me-2" />{" "}
                        </span>{" "}
                        {item.name}
                      </li>
                    ))
            ...
            <div className="row ms-5 me-5 mb-2">
              <div className="col-sm p-2">
                <span className="col-5 fw-bold p0 mb-2 fs-4">
                  Thời gian từ ngày
                </span>
                <div
                  className="col d-flex form-control"
                  onClick={() => setShowDateStart(!showDateStart)}
                >
                  <input
                    readOnly
                    type={"text"}
                    value={startNewDateTask == null ? "" : startNewDateTask}
                    style={{ border: "none", outline: "none" }}
                    className={"w-100 p-0 text-blue"}
                  />
                </div>
              </div>
              <div className="col-sm p-2">
                ...
                <div
                  className="col d-flex form-control"
                  onClick={() => setShowDateEnd(!showDateEnd)}
                >
                  <input
                    type="text"
                    readOnly
              ...
              <div className="row d-flex flex-col">
                ...
                <div className="col-sm d-flex flex-col ms-2 w-100">
                  ...
                  <div className="d-flex flex-column">
                    <div
                      className="d-flex flex-row form-control w-100 p-1"
                      onClick={() => setShowRepeatUnit(!showRepeatUnit)}
                    >
                      <input
                        value={repeatUnit}
                      ...
                      <IoMdArrowDropdown
                        className={`${
                          showRepeatUnit ? "rolated-top" : "rolated-bottom"
                        }`}
                        size={25}
                      />
                    </div>
                    {showRepeatUnit ? (
                      <div className="position-relative bg-white">
                        <div
                          ref={repeatRef}
                          className="d-flex flex-row position-absolute top-0 start-0 w-100 border border-primary translation"
                        >
                          <ul
                          ...
                          >
                            <li
                              className="list-group-item list-group-item-action"
                              onClick={() => choseRepeatUnit("Giờ")}
                            >
                              Giờ
                            </li>
                            <li
                              className="list-group-item list-group-item-action"
                              onClick={() => choseRepeatUnit("Ngày")}
                            >
                              Ngày
                            </li>
                          </ul>
            ...
            <div className="row ms-5 me-5 mb-2 mt-3">
              <div className="col-sm p-2 position-relative">
                ...
                <div
                  className="col d-flex form-control"
                  onClick={() =>
                    setShowListEmployeesInvolve(!showListEmployeesInvolve)
                  }
                >
                  <input
                    type={"search"}
                ...
                <ul className="p-3">
                  <li>
                    Có thể nhập mã hoặc tên của đề xuất /thiết bị/ công việc/
                    nhân viên/ chức vụ/ phòng ban
                  </li>
      ...
      <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <DateInput
          typenamedate={"START_DATE_NEW_TASK"}
          show={showDateStart}
          onHide={() => setShowDateStart(false)}
        />
        <DateInput
          typenamedate={"END_DATE_NEW_TASK"}
          show={showDateEnd}
          onHide={() => setShowDateEnd(false)}
        />
      </div>
    </>
  );
